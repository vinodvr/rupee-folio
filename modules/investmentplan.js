...
import { formatCurrency, formatCompact, getFunds } from './currency.js';
import {
  UNIFIED_PORTFOLIO,
  getUnifiedCategory,
...
export function initInvestmentPlan(data, curr, fh, eqAlloc, eqReturn, debtRet, arbReturn, epfRet, npsRet, epfStep, npsStep, invStep, onChange) {
  appData = data;
  currency = curr;
  fundHouse = fh || 'icici';
    ...
    <div class="bg-white rounded-lg shadow-sm p-5">
      <div class="flex flex-wrap items-center justify-between gap-y-2 mb-4">
        ...
        <div class="flex items-center gap-2">
          <span class="text-sm text-gray-600 whitespace-nowrap">Fund House:</span>
          ...
          ${shortTermSummary.totalSIP > 0 ? `
            <div class="bg-amber-50 rounded-lg p-4">
              <div class="flex justify-between items-center flex-wrap gap-y-0.5">
                <div class="min-w-0">
                  <div class="font-medium text-amber-800 break-words">${funds?.[fundHouse]?.arbitrage || 'Equity Arbitrage Fund'}</div>
            ...
            <div class="space-y-2">
              <div class="bg-green-50 rounded-lg p-3">
                ...
                <div class="space-y-2">
                  <div class="flex justify-between items-center flex-wrap gap-y-0.5">
                    <div class="min-w-0">
                      <div class="text-sm font-medium text-gray-800 break-words">${funds?.[fundHouse]?.nifty50 || 'Nifty 50 Index Fund'}</div>
                      <div class="text-xs text-gray-500">70% of equity (${Math.round(equityAllocation * 0.7)}% of SIP)</div>
                    </div>
                    <div class="font-semibold text-green-600 whitespace-nowrap">${formatCurrency(Math.round(longTermSummary.allocation.nifty50), currency)}</div>
                  </div>
                  <div class="flex justify-between items-center flex-wrap gap-y-0.5">
                    <div class="min-w-0">
                      <div class="text-sm font-medium text-gray-800 break-words">${funds?.[fundHouse]?.niftyNext50 || 'Nifty Next 50 Index Fund'}</div>
                      <div class="text-xs text-gray-500">30% of equity (${Math.round(equityAllocation * 0.3)}% of SIP)</div>
                    </div>
                    <div class="font-semibold text-green-600 whitespace-nowrap">${formatCurrency(Math.round(longTermSummary.allocation.niftyNext50), currency)}</div>
                  </div>
                </div>
              </div>
              <div class="bg-blue-50 rounded-lg p-3">
                ...
                <div class="flex justify-between items-center flex-wrap gap-y-0.5">
                  <div class="min-w-0">
                    <div class="text-sm font-medium text-gray-800 break-words">${funds?.[fundHouse]?.moneyMarket || 'Money Market Fund'}</div>
                  </div>
                  <div class="font-semibold text-blue-600 whitespace-nowrap">${formatCurrency(Math.round(longTermSummary.allocation.moneyMarket), currency)}</div>
                </div>
              </div>
            </div>
          ` : `
